...
{
	uint8* cc = (uint8*)FMemory::Malloc(len);//�����ڴ�
	app::__TcpClient->read(cc, len);//��ȡ����
	FString value = UTF8_TO_TCHAR(cc);//ת����FString
	FMemory::Free(cc);//�ͷ��ڴ�
	return value;
}
...
{
	int len = 0;
	app::__TcpClient->read(len);//��ȡ����
	if(len < 1 || len >1024*1024*5)return "";//�������С��1���ߴ���5M�����ؿ�
...
{
	FPlayerBase data;
	app::__TcpClient->read(&data, 72);//S_VECTOR是24个字节大小
